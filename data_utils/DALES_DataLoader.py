...
class DALESDataset(Dataset):
    def __init__(self, root,  pre_root, args, split='train', label_number = 8):
        super().__init__() 
        ...
        self.save_path = os.path.join(pre_root, 'dales_%s_%dpts_%s.dat' % (split, args.spoints, self.series_type))
        ...
        if not os.path.exists(self.save_path):
            ...
            rooms = [room for room in rooms if (str(self.npoints) + '_clean.npy') in room] # 
            ...
            for room_name in tqdm(rooms_split, total=len(rooms_split)):
                ...
                for i in tqdm(range(room_data.shape[0])):
                    ...
                    if args.series_type == '3dumamba_s2':
                        pc, voxel_indices, voxel_total, voxel_valid = voxelization(points, voxel_size)
                        ...
                        fps_index_array, series_idx_arrays = fps_hilbert_func(points, self.fps_n_list, self.flat)
                    
                    elif args.series_type == 'Zorder':
                        fps_index_array, series_idx_arrays = fps_zorder_func(points, self.fps_n_list, self.flat)
                    
                    elif args.series_type == 'Radial':
                        fps_index_array, series_idx_arrays = fps_radial_func(points, self.fps_n_list, self.flat)

                    elif args.series_type == 'PL':
                        fps_index_array, series_idx_arrays = fps_pl_func(points, self.fps_n_list, self.flat)
                    
                    elif args.series_type == 'Optics':
                        fps_index_array, series_idx_arrays = fps_optics_func(points, self.fps_n_list, self.flat)
                    
                    elif args.series_type == 'Random':
                        fps_index_array, series_idx_arrays = fps_random_func(points, self.fps_n_list, self.flat)

                    elif args.series_type == 'Curvature':
                        fps_index_array, series_idx_arrays = fps_curvature_func(points, self.fps_n_list, self.flat)
            ...
            with open(self.save_path, 'wb') as f:
                pickle.dump([self.sample_points, self.sample_labels, self.fps_index_array_list, self.series_idx_arrays_list, self.labelweights], f)
        else:
            print('Load processed data from %s...' % self.save_path)
            with open(self.save_path, 'rb') as f:
                self.sample_points, self.sample_labels, self.fps_index_array_list, self.series_idx_arrays_list, self.labelweights = pickle.load(f)
...
if __name__ == '__main__':
    ...
    args = parse_args()
    data_path = 'F:/Computecanada/09032024/dataset/dening/DALES/Block_s20_min4096_norm_enhance/8192_npy'
    pre_data_path = 'F:/Computecanada/09032024/dataset/dening/mamba_inverstigation/'
    ...
    for params in param_combinations:
        ...
        train_dataset = DALESDataset(root=data_path, pre_root = pre_data_path, args=args, split='train')
        test_dataset = DALESDataset(root=data_path, pre_root = pre_data_path, args=args, split='test')